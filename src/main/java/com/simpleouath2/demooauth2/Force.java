...
public class Force {
    ...
    public List<ApexClass> apexClasses(OAuth2Authentication principal) {
        String url = restUrl(principal) + "query/?q={q}";

        Map<String, String> params = new HashMap<>();
        params.put("q", "SELECT Id, Name, Body FROM ApexClass");

        return restTemplate.getForObject(url, QueryResultApexClass.class, params).records;
    }
    ...
    private static class QueryResultApexClass extends QueryResult<ApexClass> {}
